...
func (d *ModuleExampleOverviewDoc) getRelOutputPath() (string, error) {
	...
	if len(submatches) == 0 || len(submatches[0]) != IS_MODULE_EXAMPLE_OVERVIEW_DOC_REGEX_NUM_CAPTURE_GROUPS + 1 {
		return outputPath, errors.WithStackTrace(&WrongNumberOfCaptureGroupsFoundInPathRegEx{ docTypeName: "ModuleExampleOverviewDoc", path: d.relPath, regEx: IS_MODULE_EXAMPLE_OVERVIEW_DOC_REGEX })
	}
